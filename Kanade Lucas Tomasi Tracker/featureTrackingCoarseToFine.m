...
% Main function for feature tracking
    folder = '.\images';
    im = readImages(folder, 0:10:50);
    
    im1_2 = impyramid(im{1}, 'reduce');
    im1_3 = impyramid(im1_2, 'reduce');
    
    tau = 0.00001;                                 % Threshold for harris corner detection
    [pt_x, pt_y] = getKeypointsCoarse(im1_3, tau);    % Prob 1.1: keypoint detection
    
    track_flag = ones(1,numel(pt_x));
    ws = 10;                                     % Tracking ws x ws patches
...
% track_y: [Number of keypoints] x [2]
    ...
    for t = 1:nim-1
        [track_x(:, t+1), track_y(:, t+1), track_flag] = ...
                getNextPointsCoarse(track_x(:, t), track_y(:, t), im{t}, im{t+1}, ws, track_flag);
    end


function [x2, y2, track_flag] = getNextPointsCoarse(x, y, im1, im2, ws, track_flag)
    ...
    for j = 1:numel(x)        
        for i = 1:iter        
            if track_flag(j) == 1
                ...
                im1Grid = interp2(im1_3',im1GridX,im1GridY,'bilinear');
                im2Grid = interp2(im2_3',im2GridX,im2GridY,'bilinear');
